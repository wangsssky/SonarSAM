...
def main():
    ...
    val_dataset = DebrisDataset(root_path=opt.DATA_PATH, image_list=os.path.join(opt.IMAGE_LIST_PATH, 'val.txt'),
    ...
    train_loader = DataLoader(train_dataset, batch_size=opt.TRAIN_BATCHSIZE, shuffle=True, 
                              num_workers=opt.TRAIN_BATCHSIZE, collate_fn=collate_fn_seq)
    val_loader = DataLoader(val_dataset, batch_size=opt.VAL_BATCHSIZE, shuffle=False, 
                            num_workers=opt.VAL_BATCHSIZE*2, collate_fn=collate_fn_seq)    
    ...
    for epoch in range(epoch_start, epochs):
        ...
        for step, (image, mask, boxes) in enumerate(tqdm(train_loader)):
