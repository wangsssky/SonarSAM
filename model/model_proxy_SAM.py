...
class SonarSAM(nn.Module):
    ...
    def forward(self, x, boxes=None):
        ...
        if self.head_type == 'custom':
        ...
        elif self.head_type in ['semantic_mask_decoder', 'semantic_mask_decoder_LORA']:
            output = []
            for idx, curr_embedding in enumerate(out):
                ...
                output.append(low_res_masks)
            seg_out = torch.cat(output, dim=0)
...
class ModelWithLoss(nn.Module):
    def __init__(self, model):
        ...
        if self.model.num_classes > 1:
            self.dice_loss = multilabel_dice_loss
        else:
